...
> 倡議型基礎設施探針——以詮釋資料揭露六個原住民族相關資料來源之間的斷裂、可及性差異與治理空白。
...
| 視圖 | 內容 | 衡量什麼 |
|------|------|---------|
| **可及性** | 6 來源 × 6 維度評分（0-3 分） | 對外部使用者的可取用程度 |
| **治理落差矩陣** | 6 來源 × 14 維度評分（none/partial/implemented） | 對原住民族社群的權利回應 |
| **抽取 × 治理** | 2D 散點圖 | 兩個分析軸的交叉位置 |
| **資料單位對應** | 6 來源 × 5 種單位類型 | 跨來源串聯的結構基礎 |
...
- `data/sources/*.yaml` — 六個來源的結構化詮釋資料
- `data/governance-matrix.yaml` — 14 維度治理評分（84 格）
- `data/access-tiers.yaml` — 6 維度可及性評分（36 格）
